...
class ResolveSignalRequest(TypedDict):
    invocationId: str
    signalName: str
    value: str


@test_utils.handler(name="resolveSignal")
async def resolve_signal(context: Context, req: ResolveSignalRequest) -> None:
    context.resolve_signal(req["invocationId"], req["signalName"], req["value"])


class RejectSignalRequest(TypedDict):
    invocationId: str
    signalName: str
    reason: str


@test_utils.handler(name="rejectSignal")
async def reject_signal(context: Context, req: RejectSignalRequest) -> None:
    context.reject_signal(req["invocationId"], req["signalName"], req["reason"])
